...
constexpr int size = 10;   // per matrici quadrate
constexpr int r    = 7;    // target rank per Q
...
struct StageBFixture : ::testing::Test {
    ...
    Matrix Q_psd, Q_psd_rf, Q_lowrank, Q_lowrank_rf, Q_sv, Q_exp, Q_dense_rf, Q_dense_pow;
    ...
    void SetUp() override {
        ...
        Q_psd       = ARRF::adaptivePowerIteration(A_psd, tol_stageA, r, q, -1);
        Q_psd_rf    = ARRF::adaptiveRangeFinder(A_psd, tol_stageA, r, -1);

        Q_lowrank   = ARRF::adaptivePowerIteration(A_lowrank, tol_stageA, r, q, -1);
        Q_lowrank_rf= ARRF::adaptiveRangeFinder(A_lowrank, tol_stageA, r, -1);

        Q_sv        = ARRF::adaptivePowerIteration(A_sv, tol_stageA, r, q, -1);
        Q_exp       = ARRF::adaptiveRangeFinder(A_expdecay, tol_stageA, r, -1);

        Q_dense_rf  = ARRF::adaptiveRangeFinder(A_dense, tol_stageA, r, -1);
        Q_dense_pow = ARRF::adaptivePowerIteration(A_dense, tol_stageA, r, q, -1);
    }
};
} // namespace
...
TEST_F(StageBFixture, StageA_ProjectionErrorsAreSmall) {
    ...
    EXPECT_LE(Err::realError(A_expdecay,      Q_exp),        1e-2);
