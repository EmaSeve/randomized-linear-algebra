...
template<typename Derived>
typename RandomizedRangeFinder<FloatType>::CMatrix
RandomizedRangeFinder<FloatType>::fastRandomizedRangeFinder_parallel(
    const Eigen::MatrixBase<Derived>& A, int l, int seed)
{
    using Complex = std::complex<double>;
    ...
    // 5. Process each row in parallel
    #pragma omp parallel
    {
        // Thread-local FFTW setup
        std::vector<fftw_complex> row(n);
        fftw_plan plan = fftw_plan_dft_1d(n, row.data(), row.data(), FFTW_FORWARD, FFTW_ESTIMATE);
        ...
        // Cleanup thread-local FFTW plan
        fftw_destroy_plan(plan);
    }
